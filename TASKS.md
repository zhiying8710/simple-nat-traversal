...
- 当前阶段：`UDP + TCP 主链路已在本地闭环，日志控制与日常 CI 已补，继续推进 GUI TCP 入口与公网联调`
- 当前重点：`继续补齐 GUI 的 TCP 配置入口、验证 TCP 重连语义、准备真实公网联调`
- 当前边界：
  - 单密码
  - 单逻辑网络
  - UDP 打洞建立加密 P2P 会话
  ...
  - 不做虚拟网卡
- 当前已知缺口：
  - GUI / README / 示例配置仍主要按 UDP-only 表达
  - TCP 断线重连 / peer 掉线后的会话恢复语义还需要专项验证
  - GUI 服务编辑页还没有完整的 `udp/tcp` 新建入口
...
- [x] 补 TCP 本地端到端集成测试
- [ ] 验证断线重连 / peer 掉线后的 TCP 会话清理与恢复语义
...
- [ ] 验证 TCP 断线重连 / peer 掉线后的会话清理与恢复语义
- [ ] 让 GUI 服务管理页支持选择 `udp/tcp`
- [ ] 更新 `README.md` / `docs/USER_GUIDE.md` / `docs/DEPLOYMENT.md` / examples，使其反映 UDP + TCP 目标
...
- [!] 当前 GUI / 文档 / 示例与代码实现存在阶段性不一致，需要补齐
